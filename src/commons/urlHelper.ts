...
import { SETTINGS_KEY, URL_CONFIG_PATH, URL_KEY } from "./constants";
...
const getUrl = async (urlProvider = convertUrl) => {
    ...
    const urls = await http.get(URL_CONFIG_PATH),
        settingsConfig = sessionStorage.getItem(SETTINGS_KEY) && JSON.parse(sessionStorage.getItem(SETTINGS_KEY)),
        newUrls = urlProvider(urls, settingsConfig && settingsConfig.developeMode);
